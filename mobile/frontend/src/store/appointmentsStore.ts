  ...
  persist(
    function (set, get) {
      return {
        ...
        // Carga el historial de citas pasadas (completadas y canceladas)
        ...
        reset: function () {
          ...
          set({
            items: [], loading: false, hydrated: false,
            pastItems: [], loadingPast: false, hydratedPast: false,
          });
        },
      };
    },
    {
      name: '@rehabiapp/appointments-store',
      storage: createJSONStorage(() => AsyncStorage),
      partialize: (state) => ({ items: state.items, pastItems: state.pastItems }),
    }
  )
);
